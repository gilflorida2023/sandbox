...
## This Run — Graphical GFM Viewer (Java)

Current task: build a **graphical GFM rendering tool in Java (JavaFX)** in the
`gfm-viewer/` project. The target goal is in the goal spec; the GFM language
spec is an *additional reference* spec used for conformance testing.

### Spec files (this run)
| Path | Purpose |
|------|---------|
| `workspace/specs/GOAL-build-gfm-graphical-viewer-java.md` | **Target goal spec** for this run (what we build & how). Read this FIRST. |
| `workspace/specs/gfm-language/*` (44 chapters) + `index.json` | **GFM language specification** — an *additional/reference* spec, NOT the project. Drives `gfm.SpecExampleTest`. Do not read all by default. |

### Implementation plan (this run)
- `workspace/IMPLEMENTATION_PLAN.md` — task list for this run. Regenerated by
  `loop.sh --clean` from the specs in `workspace/specs/`.

### Permanent vs task-specific files
**Permanent** (agent infra + Java project; persist across runs; NOT removed by
`loop.sh --clean`):
- Agent core: `README.md`, `AGENTS.md`, `PROMPT_build.md`, `PROMPT_plan.md`,
  `loop.sh`, `ralph-agent.sh`, `mcp_tool.sh`, `tool_definitions.json`, `build.sh`, `llm.sh`
- Tool server: `scout/` (CGI tools under `scout/cgi-bin/workspace/`)
- Java project source: `gfm-viewer/` (`src/`, `pom.xml`)

**Task-specific / generated** (this run; under `workspace/` or build output;
regenerated or safe to delete):
- `workspace/specs/GOAL-build-gfm-graphical-viewer-java.md` — this run's goal spec
- `workspace/specs/gfm-language/*` + `index.json` — ingested GFM language spec (reference data)
- `workspace/IMPLEMENTATION_PLAN.md` — this run's task list
- `workspace/ollama_api_log.jsonl` — Ralph's Ollama API call log
- `gfm-viewer/target/` — Maven build output (jar, classes, surefire reports)
